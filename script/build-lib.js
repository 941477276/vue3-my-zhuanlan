...
function doTsc (globby, targetDir) {
  ...
  let startTime = new Date().getTime();
  let tsFilePaths = globby.globbySync('(src/**/*-types.ts)|src/**/type.js|src/utils/*.ts|src/hooks/*.ts');
  // console.log(tsFilePaths);
  // 生成ts类型文件
  nodeCMD.runSync(`tsc ${tsFilePaths.join(' ')} --declaration --emitDeclarationOnly`);
  // 复制ts类型文件到目标目录
  globby.globbySync('src/**/*.d.ts').forEach(filePath => {
    ...
    utils.copy(filePath, filePath.replace('src/', targetDir + '/'));
