...
## zh-CN
`bs-vue`提供了`bs-checkbox-group`组件来将多个多选选按钮进行组合，组合后无需在每一个`bs-checkbox`组件中都绑定`v-model`，
而只需在`bs-checkbox-group`组件上绑定`v-model`，在`bs-checkbox`中设置`value`即可

## en-US
`bs-vue` provides the `bs-checkbox-group` component to combine multiple checkbox buttons. After the combination, there is no need to bind `v-model` in each `bs-checkbox` component.
Instead, just bind `v-model` on the `bs-checkbox-group` component and set `value` in `bs-checkbox`
</docs>
