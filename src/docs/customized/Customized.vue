...
export function TestIcon4CircleFill () {
  ...
  return createVNode(BsIcon, {
    iconName: 'test-icon-4-circle-fill', // 图标名称
    isFilled: true, // 图标是否为填充类型的
    viewBox: svgVDom.attrs.viewBox, // svg的viewBox属性
    svgChildrenVDom: svgVDom.children, // svg的子内容
    // 'class': 'test-icon'
  });
}
</pre>
      <h2>使用bs-icon-vue生成工具生成自定义图标</h2>
      假设我们的项目目录结构是这样的：
...
/*  // 单个构建
generateIcons({
  svgFilePath: path.resolve(__dirname, './src/svg/filled/1-circle-outlined.svg'), // svg文件路径
  outfile: path.resolve(__dirname, './src/components/icons/1-circle-outlined.js'), // 图标文件输出路径
  // outdir: path.resolve(__dirname, './src/components/icons'), // 图标文件输出目录，outdir与outfile任选一
...
// 批量构建
generateIcons({
  svgDir: path.resolve(__dirname, './src/svgs'), // 存放svg目录
  outdir: path.resolve(__dirname, './src/components/icons'), // 图标文件输出目录
